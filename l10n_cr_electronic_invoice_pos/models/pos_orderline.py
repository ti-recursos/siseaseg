...
class PosOrderLinesExtended(models.Model):
    ...
    exclude_tax = fields.Selection(
        selection=[('0', 'Normal'),
                   ('1', 'Cargo a la Habitación')],
        string="Cargo a la Habitación",
    ...
    def create(self, values):
        ...
        try:
            if values.get('exclude_tax'):
                values['exclude_tax'] = str(values.get('exclude_tax'))
        except Exception:
            values['exclude_tax'] = 0
            pass
    ...
    def create_picking(self):
        ...
        for order in self:
            ...
            for line in order.lines.filtered(lambda l: l.product_id.type in ['product', 'consu'] and not float_is_zero(l.qty, precision_rounding=l.product_id.uom_id.rounding)):
                ...
                moves |= Move.create({
                    'name': line.name,
                    'exclude_tax': str(line.exclude_tax) or '0',
