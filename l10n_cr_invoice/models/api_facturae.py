...
def get_invoice_attachments(invoice, record_id):
    ...
    if attachment.id:
        ...
        attach_copy = invoice.env['ir.attachment'].sudo().create({'name': invoice.fname_xml_comprobante,
                                                           'type': 'binary',
    ...
    if attachment_resp.id:
        ...
        attach_resp_copy = invoice.env['ir.attachment'].sudo().create({'name': invoice.fname_xml_respuesta_tributacion,
                                                                'type': 'binary',
    ...
    if inv.tipo_documento != 'FEC' and estado_m_h == 'aceptado' and (last_state is False or last_state == 'procesando'):
        ...
        if inv.move_type == 'in_invoice' or inv.move_type == 'in_refund':
            email_template = self.env.ref(
                'l10n_cr_invoice.email_template_invoice_vendor', False)
        else:
            email_template = self.env.ref(
        ...
        if len(attachments) == 2:
            _logger.debug('FECR - attachments:',attachments)
