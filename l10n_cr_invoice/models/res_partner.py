...
class PartnerElectronic(models.Model):
    ...
    invoice_email = fields.Char(string='Correo Factura Electrónica')

    @api.onchange('invoice_email')
    def validate_invoice_email(self):
        if self.invoice_email:
            mails = self.invoice_email.split(',')
            for mail in mails:
                match = re.match('^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$', mail)
                if match == None:
                    raise ValidationError('El correo electrónico no es válido: %s' % mail)
