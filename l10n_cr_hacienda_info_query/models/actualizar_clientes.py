...
class res_partner(models.Model):
    ...
    def definir_informacion(self,cedula):
        url_base = self.sudo().env.company.url_base
        if url_base:
            ...
            if peticion.status_code in (200,202) and len(peticion._content) > 0:
                ...
                self.sudo().env.company.ultima_respuesta = ultimo_mensaje
                ...
                if 'nombre' in contenido:
                    if 'identification_id' in self._fields:
                        if 'tipoIdentificacion' in contenido:
                            ...
                            if clasificacion == '01':#Cedula Fisica
                                self.identification_id = self.env['identification.type'].sudo().search([('code', '=', '01')], limit=1).id
                            elif clasificacion == '02':#Cedula Juridica
                                self.identification_id = self.env['identification.type'].sudo().search([('code', '=', '02')], limit=1).id
                            elif clasificacion == '03':#Cedula Juridica
                                self.identification_id = self.env['identification.type'].sudo().search([('code', '=', '03')], limit=1).id
                            elif clasificacion == '04':#Cedula Juridica
                                self.identification_id = self.env['identification.type'].sudo().search([('code', '=', '04')], limit=1).id
                            elif clasificacion == '05':#Cedula Juridica
                                self.identification_id = self.env['identification.type'].sudo().search([('code', '=', '05')], limit=1).id
                    ...
                    if contenido.get('nombre') != None:
                       ...
                       for act in actividades:
                          if act.get('estado') == 'A':
                              fe = self.env['ir.module.module'].sudo().search([('name', '=', 'l10n_cr_invoice')])
                              if fe.state == 'installed':
                                  self.activity_id = self.env['economic.activity'].sudo().search([('code', '=', str(act.get('codigo')))], limit=1).id
            # else:
    ...
    def onchange_vat(self):
        if not self.vat:
            ...
            if self.vat != '' and self.vat != '111111111':
