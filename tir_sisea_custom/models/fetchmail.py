...
class FetchmailServer(models.Model):
    ...
    def fetch_mail(self):
        _logger.error('Fetchmail de TIR SISEA')
            ...
            if server.server_type == 'imap':
                try:
                    ...
                    for num in data[0].split():
                        ...
                        try:
                            if server.type_bank:
                                res_id = MailThread.with_context(**additionnal_context).message_process_bank(
                                    server.object_id.model, data[0][1], save_original=server.original,
                                    strip_attachments=(not server.attach))
                            elif server.invoice_receptor:
                                res_id = MailThread.with_context(**additionnal_context).message_process_fe(server.object_id.model, data[0][1], save_original=server.original, strip_attachments=(not server.attach))
                            else:
